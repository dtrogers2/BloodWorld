...
public class HitCmd : CmdBase
{
    ...
    public void doDmg(int dmg, bool hit, uint tgt, IGame game)
    {
        ...
        Msg m = new Msg { color = (dmg > 0 && hit ) ? ColorHex.White: ColorHex.GrayDark, text = s};
        game.msg(m);
    ...
    {
        int dmg = 0;
        if (ENTITY.has(me, COMPONENT.ATTACK)) {
            Attacks a = (Attacks) ComponentManager.get(COMPONENT.ATTACK).data[me];
